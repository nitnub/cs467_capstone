...
int sdlVideoInit(Game *game) {
    ...
    // create the texture that the game screen will be drawn on
    // (using a texture allows resizing)
    game->texture = SDL_CreateTexture(game->renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, PIXEL_WIDTH, PIXEL_HEIGHT);
    if (!game->texture) {
...
void populateSetBitBuffer(char **result, uint16_t number) {
    int pos = 0;
...
int updatePoints(Points_t *points) {
    ...
    for (int i = 0; i < sizeof(testVals) / sizeof(testVals[0]); i++) {
        ...
        populateSetBitBuffer(&binBuffer, testVals[i]);
